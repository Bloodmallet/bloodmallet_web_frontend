...
class Simulation(models.Model):
    ...
    custom_apl = models.TextField(
        max_length=65536,
        blank=True,
        help_text=_("Define your characters APL."),
    )
...
class Queue(models.Model):
    ...
    state = models.CharField(
        max_length=20, choices=[(tag.name, tag.value) for tag in QueueState]
    )
...
class GeneralResult(models.Model):
    ...
    wow_class = models.ForeignKey(
        WowClass,
        on_delete=models.CASCADE,
        related_name="general_results",
    )
    wow_spec = models.ForeignKey(
        WowSpec,
        on_delete=models.CASCADE,
        related_name="general_result",
    )
    simulation_type = models.ForeignKey(
        SimulationType,
        on_delete=models.CASCADE,
